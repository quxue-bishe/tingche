<template>
  <div class="container">
    <Breadcrumb
      :items="['menu.pay', 'menu.pay.bank', 'menu.pay.bank.bankBranchList']"
    />
    <a-card class="general-card" :title="$t('menu.pay.bank.bankBranchList')">
      <a-row>
        <a-col :flex="1">
          <a-form
            :model="formModel"
          ...
          >
            <a-row :gutter="16">
              ...
              <a-col :span="8">
                <a-form-item field="interbankNumber" label="行联号">
                  <a-input
                    v-model="formModel.interbankNumber"
                    placeholder="请输入"
                    allow-clear
                  />
                </a-form-item>
...
<script lang="ts" setup>
  ...
  import { getBankBranchListByQuery, deleteBankBranch } from '@/api/pay';
  ...
  const onDelete = async (id: string) => {
    await deleteBankBranch({ params: { bankId: id } });
    fetchData();
  };
  ...
  const fetchData = async () => {
    loading.value = true;
    // @ts-ignore
    const { data, code, total } = await getBankBranchListByQuery({
      params: checkParams({
        page: pagination.current,
        size: pagination.pageSize,
        bankName: formModel.value.bankName,
        interbankNumber: formModel.value.interbankNumber,
      }),
    });
    loading.value = false;
    if (code === 10002) {
