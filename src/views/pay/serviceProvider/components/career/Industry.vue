...
<script lang="ts" setup>
  ...
  onMounted(() => {
    if (localStorage.getItem('career-Industry')) {
      // @ts-ignore
      formData.value = JSON.parse(localStorage.getItem('career-Industry'));
    }
  });
  ...
  const onNext = () => {
    formRef.value.validate((valid: any) => {
      ...
      // @ts-ignore
      localStorage.setItem('career-Industry', JSON.stringify(formData.value));
      emit('changeStep', 4);
      return true;
    });
  };
