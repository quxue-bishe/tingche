...
<script lang="ts" setup>
  ...
  onMounted(() => {
    if (localStorage.getItem('career-Settlement')) {
      // @ts-ignore
      formData.value = JSON.parse(localStorage.getItem('career-Settlement'));
    }
  });
  ...
  const onNext = () => {
    formRef.value.validate((valid: any) => {
      ...
      // @ts-ignore
      localStorage.setItem('career-Settlement', JSON.stringify(formData.value));
      emit('changeStep', 5);
      return true;
    });
  };
