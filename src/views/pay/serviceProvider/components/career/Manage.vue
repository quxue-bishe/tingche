...
<script lang="ts" setup>
  ...
  onMounted(() => {
    if (localStorage.getItem('career-Manage')) {
      // @ts-ignore
      formData.value = JSON.parse(localStorage.getItem('career-Manage'));
    }
  });
  ...
  const onNext = () => {
    formRef.value.validate((valid: any) => {
      ...
      // @ts-ignore
      localStorage.setItem('career-Manage', JSON.stringify(formData.value));
      emit('changeStep', 3);
      return true;
    });
  };
