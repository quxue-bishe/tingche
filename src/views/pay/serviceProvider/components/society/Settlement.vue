...
<script lang="ts" setup>
  ...
  onMounted(() => {
    if (localStorage.getItem('society-Settlement')) {
      // @ts-ignore
      formData.value = JSON.parse(localStorage.getItem('society-Settlement'));
    }
    getBankList();
  });
  ...
  const onNext = () => {
    formRef.value.validate((valid: any) => {
      ...
      // @ts-ignore
      localStorage.setItem('society-Settlement', JSON.stringify(formData.value));
      emit('changeStep', 5);
      return true;
    });
  };
