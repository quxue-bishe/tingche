<template>
  <a-form ref="formRef" :model="formData" style="margin-top: 32px">
    ...
    <a-form-item field="contact_id_number" label="证件号码" required>
...
<script lang="ts" setup>
  ...
  onMounted(() => {
    if (localStorage.getItem('society-SuperManagement')) {
      // @ts-ignore
      formData.value = JSON.parse(localStorage.getItem('society-SuperManagement'));
    }
  });
  ...
  const onNext = () => {
    formRef.value.validate(async (valid: any) => {
      ...
      // @ts-ignore
      localStorage.setItem('society-SuperManagement', JSON.stringify(formData.value));
      const paramsInfo = {
        bank_account_info: JSON.parse(localStorage.getItem('society-Settlement')),
        subject_info: JSON.parse(localStorage.getItem('society-Subject')),
        business_info: JSON.parse(localStorage.getItem('society-Manage')),
        settlement_info: JSON.parse(localStorage.getItem('society-Industry')),
        contact_info: JSON.parse(localStorage.getItem('society-SuperManagement')),
      };
      console.log(
        '🚀 ~ file: SuperManagement.vue:78 ~ formRef.value.validate ~ paramsInfo:',
        paramsInfo
      );
      const { data, code } = await wxInboundApplicationOfSpecialMerchants(paramsInfo)
      console.log("🚀 ~ file: SuperManagement.vue:79 ~ formRef.value.validate ~ data, code:", data, code)
      return true;
    });
    console.log('完成');
  };
