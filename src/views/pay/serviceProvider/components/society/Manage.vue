...
<script lang="ts" setup>
  ...
  onMounted(() => {
    if (localStorage.getItem('society-Manage')) {
      // @ts-ignore
      formData.value = JSON.parse(localStorage.getItem('society-Manage'));
    }
  });
  ...
  const onNext = () => {
    formRef.value.validate((valid: any) => {
      ...
      // @ts-ignore
      localStorage.setItem('society-Manage', JSON.stringify(formData.value));
      emit('changeStep', 3);
      return true;
    });
  };
