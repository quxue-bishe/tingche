...
<script lang="ts" setup>
  ...
  onMounted(() => {
    if (localStorage.getItem('enterprise-Manage')) {
      // @ts-ignore
      formData.value = JSON.parse(localStorage.getItem('enterprise-Manage'));
    }
  });
  ...
  const onNext = () => {
    formRef.value.validate((valid: any) => {
      ...
      // @ts-ignore
      localStorage.setItem('enterprise-Manage', JSON.stringify(formData.value));
      emit('changeStep', 3);
      return true;
    });
  };
