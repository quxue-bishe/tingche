...
<script lang="ts" setup>
  ...
  onMounted(() => {
    if (localStorage.getItem('enterprise-Industry')) {
      // @ts-ignore
      formData.value = JSON.parse(localStorage.getItem('enterprise-Industry'));
    }
  });
  ...
  const onNext = () => {
    formRef.value.validate((valid: any) => {
      ...
      // @ts-ignore
      localStorage.setItem('enterprise-Industry', JSON.stringify(formData.value));
      emit('changeStep', 4);
      return true;
    });
  };
