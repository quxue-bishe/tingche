<template>
  <a-form ref="formRef" :model="formData" style="margin-top: 32px">
    <a-form-item field="activityName" label="账户类型" required>
      <a-radio-group>
...
<script lang="ts" setup>
  ...
  onMounted(() => {
    if (localStorage.getItem('individual-Settlement')) {
      // @ts-ignore
      formData.value = JSON.parse(localStorage.getItem('individual-Settlement'));
    }
  });
  ...
  const onNext = () => {
    formRef.value.validate((valid: any) => {
      ...
      // @ts-ignore
      localStorage.setItem('individual-Settlement', JSON.stringify(formData.value));
      emit('changeStep', 5);
      return true;
    });
  };
