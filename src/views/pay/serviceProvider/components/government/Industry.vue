...
<script lang="ts" setup>
  ...
  onMounted(() => {
    if (localStorage.getItem('government-Industry')) {
      // @ts-ignore
      formData.value = JSON.parse(localStorage.getItem('government-Industry'));
    }
  });
  ...
  const onNext = () => {
    formRef.value.validate((valid: any) => {
      ...
      // @ts-ignore
      localStorage.setItem('government-Industry', JSON.stringify(formData.value));
      emit('changeStep', 4);
      return true;
    });
  };
