  ...
  <div class="container">
    <Breadcrumb
      :items="[
        'menu.ucenter',
        'menu.ucenter.role',
        'menu.ucenter.roleproduct.list',
      ]"
    />
    <a-card class="general-card" :title="$t('menu.ucenter.roleproduct.list')">
      <a-row>
        <a-col :flex="1">
          <a-form
            :model="formModel"
          ...
          >
            <a-row :gutter="16">
              <a-col :span="8">
                <a-form-item field="roleKey" label="角色Key">
                  <a-input
...
<script lang="ts" setup>
  ...
  import {
    getUserRoleProductListByQuery,
    deleteUserRoleProduct,
  } from '@/api/user';
  ...
  const onDelete = async (id: string) => {
    await deleteUserRoleProduct({ params: { id } });
    fetchData();
  };
  ...
  const fetchData = async () => {
    loading.value = true;
    // @ts-ignore
    const { data, code, total } = await getUserRoleProductListByQuery({
      params: checkParams({
    ...
    if (code === 10002) {
      ...
      for (const i of data) {
        i.newPackageUnit = getFrequencyText(i.packageUnit);
      }
      state.list = data;
    } else {
      state.list = [];
