<template>
  <div class="container">
    <Breadcrumb
      :items="[
        'menu.ucenter',
        'menu.ucenter.department',
        'menu.ucenter.department.list',
      ]"
    />
    <a-card class="general-card" :title="$t('menu.ucenter.department.list')">
      <a-row>
        <a-col :flex="1">
          <a-form
            :model="formModel"
...
<script lang="ts" setup>
  ...
  import { cfDepartmentGet, cfDepartmentDelete } from '@/api/user';
  ...
  const fetchData = async () => {
    loading.value = true;
    // @ts-ignore
    const { data, code, total } = await cfDepartmentGet({
      params: checkParams({
