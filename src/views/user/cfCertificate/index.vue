  ...
  <div class="container">
    <Breadcrumb
      :items="[
        ...
        'menu.ucenter.certificate.idcardlist',
      ]"
    />
    <a-card class="general-card" :title="$t('menu.ucenter.certificate.idcardlist')">
      <a-row>
        <a-col :flex="1">
          <a-form
            :model="formModel"
          ...
          >
            <a-row :gutter="16">
              ...
              <a-col :span="8">
                <a-form-item field="identityNumber" label="身份证号">
                  <a-input
                    v-model="formModel.identityNumber"
                    placeholder="请输入身份证号"
                    allow-clear
                  />
                </a-form-item>
...
<script lang="ts" setup>
  ...
  import { cfUserIdCardGet, cfUserIdCardDelete } from '@/api/user';
  ...
  const onDelete = async (id: string) => {
    await cfUserIdCardDelete({ params: { id } });
    fetchData();
  };
  ...
  const fetchData = async () => {
    loading.value = true;
    // @ts-ignore
    const { data, code, total } = await cfUserIdCardGet({
      params: checkParams({
        page: pagination.current,
        size: pagination.pageSize,
        userName: formModel.value.userName,
        identityNumber: formModel.value.identityNumber,
      }),
    });
    loading.value = false;
    console.log(data);
    ...
    if (code === 10002) {
      ...
      for (const i of data) {
        ...
        i.newSex = i.sex == 0 ? '女' : i.sex == 1 ? '男' : '未知';
      }
      state.list = data;
    } else {
      state.list = [];
