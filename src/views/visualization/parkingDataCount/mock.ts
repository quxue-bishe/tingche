...
setupMock({
  setup() {
    Mock.mock(
      ...
      (params: PostData) => {
        ...
        return successResponseWrap({
          ...
          chartData: [
            // itemStyle for demo
            { name: '智慧停车', value: 25, itemStyle: { color: '#8D4EDA' } },
            { name: '充电', value: 35, itemStyle: { color: '#165DFF' } },
            { name: '物业缴费', value: 40, itemStyle: { color: '#00B2FF' } },
          ],
        });
      }
    );
    ...
    Mock.mock(new RegExp('/api/content-publish'), () => {
      ...
      return successResponseWrap([
        generateLineData('智慧停车'),
        generateLineData('新能源充电'),
        generateLineData('物业缴费'),
      ]);
    });
