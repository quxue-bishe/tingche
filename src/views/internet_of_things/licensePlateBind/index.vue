  ...
  <div class="container">
    <Breadcrumb
      :items="[
        ...
        'menu.pay.internetofthings.carparkcar',
      ]"
    />
    <a-card class="general-card" :title="$t('menu.pay.internetofthings.carparkcar')">
      <a-row>
        <a-col :flex="1">
          <a-form
            :model="formModel"
          ...
          >
            <a-row :gutter="16">
              ...
              <a-col :span="8">
                <a-form-item field="vin" label="vin">
                  <a-input
...
<script lang="ts" setup>
  ...
  import { bindGet, bindDelete } from '@/api/internetOfThings';
  ...
  const fetchData = async () => {
    loading.value = true;
    // @ts-ignore
    const { data, code, total } = await bindGet({
      params: checkParams({
        page: pagination.current,
        size: pagination.pageSize,
        numberPlate: formModel.value.numberPlate,
        vin: formModel.value.vin,
      }),
    });
    loading.value = false;
    if (code === 10002) {
