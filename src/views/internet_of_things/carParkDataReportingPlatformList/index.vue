  ...
  <div class="container">
    <Breadcrumb
      :items="[
        ...
        'menu.pay.internetofthings.carpark.DataReportingPlatformList',
      ]"
    />
    <a-card
      class="general-card"
      :title="$t('menu.pay.internetofthings.carpark.DataReportingPlatformList')"
    >
      <a-row>
        <a-col :flex="1">
          <a-form
...
<script lang="ts" setup>
  ...
  import {
    ...
    deleteCarParkDataReportingPlatform,
  ...
  const onDelete = async (id: string) => {
    const { message, code } = await deleteCarParkDataReportingPlatform({
      params: { id },
    });
    if (code === 10002) {
      fetchData();
    ...
    const { data, code, total } =
      await getCarParkDataReportingPlatformListByQuery({
        params: checkParams({
          ...
          platformName: formModel.value.platformName,
        }),
      });
    loading.value = false;
    if (code === 10002) {
